...
jq( document ).ready( function(){
    ...
    var productPage = jq( 'body' ).hasClass( 'single-product' );
         ...
         * 
         */
        var get_yt_wrap_class               = jq( '.product-gallery li.youtube-popup' );

        if( get_yt_wrap_class.length > 0 ){
            var get_yt_video_url                = get_yt_wrap_class.find('a').attr( 'href' ).match( /youtube.com\/watch\?v=(.{11})/ );
            var get_yt_video_id                 = get_yt_video_url.pop();
            var get_yt_video_featured_img_src   = 'https://img.youtube.com/vi/' + get_yt_video_id + '/hqdefault.jpg';
        ...
        jq( '.single_variation_wrap' ).on( 'show_variation', function(event, variation){
            ...
            $product_img_wrap.attr( 'class', function(){
        ...
        jq( '.reset_variations' ).on( 'click', function(){
            ...
            if( get_yt_wrap_class.length > 0 ){
                $product_img_wrap.attr( 'class', 'woocommerce-product-gallery__image' ).css({
                    'opacity': 0,
                    'z-index': 1
                })
            }
